...
df = pd.DataFrame(finalList, columns= ["Weight", "FT", "Counter"])

m, b = np.polyfit(df.FT, df.Weight, 1)
fitEquation = m*df.FT + b
...
z = np.polyfit(df.Weight, df.FT, 1)
p = np.poly1d(z)
...
plt.ylabel('FT%', fontweight='bold')
plt.title("Weight VS Free Throw %", fontweight="bold", fontsize=20)
