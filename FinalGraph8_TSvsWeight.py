...
for n in range(1, len(dataPercents)):
    ...
    if float(dataPercents[n][5]) >= 10 and float(dataPercents[n][13]) >= 40: 
        while p < len(dataHeightsWeights) and name != dataHeightsWeights[p][0]: 
            p += 1

        if p<len(dataHeightsWeights):
            output = float(dataPercents[n][14]) * 100
...
df = pd.DataFrame(finalList, columns= ["Weight", "Three", "Counter"])

m, b = np.polyfit(df.Three, df.Weight, 1)
fitEquation = m*df.Three + b
...
plt.scatter(df.Weight,df.Three, s = df.Counter, c=np.random.rand(len(df.Weight),3), alpha = 0.5)
plt.text(220, 94, "Size of dot represents number of players at specific weight")
z = np.polyfit(df.Weight, df.Three, 1)
p = np.poly1d(z)
...
plt.ylabel('3PT%', fontweight='bold')
plt.title("Weight VS Three Point %", fontweight="bold", fontsize=20)
